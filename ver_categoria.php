...
elseif(isset($_GET['s'])){
    $sub_cat_qry = $conn->query("SELECT * FROM sub_categorias where md5(id) = '{$_GET['s']}'");
    if($sub_cat_qry->num_rows > 0){
        $title = $sub_cat_qry->fetch_assoc()['sub_categoria'];
    }
}
?>
<!-- Header-->
...
<section class="py-5">
    <div class="container px-4 px-lg-5 mt-5">
        <div class="row gx-2 gx-lg-5 row-cols-1 row-cols-md-2 row-cols-xl-2 justify-content-center">
            ...
            <?php 
                $whereData = "";
                $categorias = $conn->query("SELECT * FROM `categorias` where status = 1 order by categoria asc ");
            ...
            <div class="col mb-6 mb-2">
                <a href="./?p=productos&c=<?php echo md5($row['id']) ?>" class="card categoria-item text-dark">
                    <div class="card-body p-4">
                        <div class="">
                            <!-- Product name-->
                            <h5 class="fw-bolder border-bottom border-primary"><?php echo $row['categoria'] ?></h5>
                        </div>
