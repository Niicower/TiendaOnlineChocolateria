...
<?php 
$Categoria = isset($_GET['b']) ? json_decode(urldecode($_GET['b'])) : array();
?>
<section class="py-0">
    ...
    <div class="row">
        <div class="col-lg-2 px-1 border-right text-sm position-sticky ">
            ...
            <ul class="list-group">
                ...
                <?php 
                $qry = $conn->query("SELECT * FROM Categoria where status =1 order by name asc");
                while($row=$qry->fetch_assoc()):
                ?>
                <li class="list-group-item list-group-item-action">
                    <div class="icheck-primary d-inline">
                        <input type="checkbox" id="brand-item-<?php echo $row['id'] ?>" <?php echo in_array($row['id'],$Categoria) ? "checked" : "" ?> class="brand-item" value="<?php echo $row['id'] ?>">
            ...
            <div class="container px-4 px-lg-5 mt-5">
                <div class="row gx-4 gx-lg-4 row-cols-md-3 row-cols-xl-4 ">
                    <?php 
                        ...
                        $where = " and p.categoria_id in (".implode(",",$Categoria).") " ;
                        $productos = $conn->query("SELECT p.*,b.name as bname FROM `productos` p inner join Categoria b on p.categoria_id = b.id where p.status = 1 {$where} order by rand() ");
