...
class SystemSettings extends DBConnection{
	...
	function update_system_info(){
		...
		$qry = $this->conn->query($sql);
			while($row = $qry->fetch_assoc()){
				if(isset($_SESSION['info_del_sistema'][$row['meta_field']]))unset($_SESSION['info_del_sistema'][$row['meta_field']]);
				$_SESSION['info_del_sistema'][$row['meta_field']] = $row['meta_value'];
			}
		return true;
	}
	function update_settings_info(){
		...
		foreach ($_POST as $key => $value) {
			...
			if(isset($_SESSION['info_del_sistema'][$key])){
				...
				$qry = $this->conn->query("UPDATE info_del_sistema set meta_value = '{$value}' where meta_field = '{$key}' ");
			}else{
				$qry = $this->conn->query("INSERT into info_del_sistema set meta_value = '{$value}', meta_field = '{$key}' ");
			}
		}
		...
		if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
			...
			if(isset($_SESSION['info_del_sistema']['logo'])){
				$qry = $this->conn->query("UPDATE info_del_sistema set meta_value = '{$fname}' where meta_field = 'logo' ");
				if(is_file('../'.$_SESSION['info_del_sistema']['logo'])) unlink('../'.$_SESSION['info_del_sistema']['logo']);
			}else{
				$qry = $this->conn->query("INSERT into info_del_sistema set meta_value = '{$fname}',meta_field = 'logo' ");
			}
		}
		if(isset($_FILES['cover']) && $_FILES['cover']['tmp_name'] != ''){
			...
			if(isset($_SESSION['info_del_sistema']['cover'])){
				$qry = $this->conn->query("UPDATE info_del_sistema set meta_value = '{$fname}' where meta_field = 'cover' ");
				if(is_file('../'.$_SESSION['info_del_sistema']['cover'])) unlink('../'.$_SESSION['info_del_sistema']['cover']);
			}else{
				$qry = $this->conn->query("INSERT into info_del_sistema set meta_value = '{$fname}',meta_field = 'cover' ");
			}
		}
